...
def process_video(dwprocessor, video_path, output_video_path, detect_resolution):
    ...
    video_writer = cv2.VideoWriter(
        output_video_path, cv2.VideoWriter_fourcc(*"mp4v"), fps, size
    )
...
dwprocessor = DWposeDetector()
dataset_folder = "/home/ubuntu/data/ubc_fashion"
...
for sub_folder in sub_folders:
    ...
    for video_name in tqdm(os.listdir(path), desc=f"Processing {sub_folder}"):
        ...
        output_video_path = os.path.join(
            output_folder, video_name.split(".")[0] + ".mp4"
        )
        if os.path.exists(output_video_path): continue
