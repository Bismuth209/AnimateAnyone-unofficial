...
class TikTok(Dataset):
    ...
    def get_batch(self, idx):
        ...
        video_pose_dir = os.path.join(
            self.video_folder, folder_name, f"{folder_name}_dwpose.mp4"
        )
        ...
        assert len(video_reader) == len(
            video_reader_pose
        ), f"len(video_reader) != len(video_reader_pose) in video {idx}"
        ...
        pixel_values_pose = (
            torch.from_numpy(video_reader_pose.get_batch(batch_index).asnumpy())
            .permute(0, 3, 1, 2)
            .contiguous()
        )
        ...
        pixel_values_ref_img = (
            torch.from_numpy(ref_img.asnumpy()).permute(2, 0, 1).contiguous()
        )
...
class UBC_Fashion(Dataset):
    ...
    def get_batch(self, idx):
        ...
        video_pose_dir = os.path.join(
            self.video_folder, self.spilt + "_dwpose", f"{folder_name}.mp4"
        )
