...
from models.ReferenceNet import ReferenceNet as UNet2DConditionModel
...
class AnimateAnyonePipeline(DiffusionPipeline):
    ...
    def _encode_pose_sequence(
        ...
        pose_sequence = pose_sequence.to(device)  # bs f 3 h w
        batch_size, num_frames, channel, height, width = pose_sequence.shape
        pose_sequence = pose_sequence.reshape(
            batch_size * num_frames, channel, height, width
        )  # (bs f) 3 h w
        pose_embeddings = self.pose_guider(pose_sequence)  # (bs f) 3 h w
        pose_embeddings = pose_embeddings.repeat(
            num_videos_per_prompt, 1, 1, 1
        )  # ( ??? )
    ...
    def __call__(
        ...
        pose_sequence: Union[
            List[PIL.Image.Image], List[List[PIL.Image.Image]], torch.FloatTensor
        ],
        height: int = 576,
        width: int = 1024,
        ...
        if isinstance(image, PIL.Image.Image):
            ...
            if isinstance(pose_sequence[0], list):
                ...
                if batch_size != pose_batch_size:
                    raise ValueError(
                        "batch sizes must be the same for the reference images and pose sequences"
                    )
        else:
