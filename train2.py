...
from models.unet import UNet3DConditionModel, load_unet_with_motion_module_state_dict
...
def load_all_models(
    ...
    **kwargs,
    ...
    if not image_finetune:
        referencenet_and_poseguider_state_dict = torch.load(
            referencenet_and_poseguider_checkpoint_path, map_location="cpu"
        )

        try:
            poseguider.load_state_dict(
                referencenet_and_poseguider_state_dict["poseguider_state_dict"],
                strict=True,
            )
        except Exception as e:
            Warn(
                f"Could not load weights to poseguider from {referencenet_and_poseguider_checkpoint_path}\nError: {e}"
            )

        try:
            referencenet.load_state_dict(
                referencenet_and_poseguider_state_dict["referencenet_state_dict"],
                strict=True,
            )
            Warn(
                f"Could not load weights to referencenet from {referencenet_and_poseguider_checkpoint_path}\nError: {e}"
            )
        except Exception as e:
            Warn(e)
    ...
    if motion_checkpoint_path is not None:
        load_unet_with_motion_module_state_dict(unet, motion_checkpoint_path)
